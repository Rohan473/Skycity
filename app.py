...
def prepare_ml(df):
    ...
    # ── CORE FEATURES (removed 11 redundant features)
    # Removed: RevenuePerOrder (r=1.0 w/ AOV), GrowthAdj_Orders (r=0.998 w/ MonthlyOrders),
    #          InStoreRevRatio, UE_RevRatio, DD_RevRatio, SD_RevRatio (derived from shares),
    #          Commission_UE, DeliveryCost_SD (derived), CostToRevenue (r=0.895 w/ OPEX, 0.840 w/ COGS),
    #          ProfitPerOrder (target leakage), DeliveryRadiusKM (r=1.0 w/ DeliveryCostPerOrder)
    FEATURES = [
        # Channel Mix (must keep all 4 - sum to 1.0)
        "InStoreShare", "UE_share", "DD_share", "SD_share",
        
        # Pricing & Cost Structure
        "CommissionRate", "DeliveryCostPerOrder", "COGSRate", "OPEXRate",
...
def prepare_ml_target(df, target):
    ...
    # ── CORE FEATURES (matching prepare_ml cleanup)
    FEATURES = [
        # Channel Mix (must keep all 4 - sum to 1.0)
        "InStoreShare", "UE_share", "DD_share", "SD_share",
...
def build_feature_row(
    ...
    # ── CORE FEATURES ONLY (removed redundant derived features)
    return np.array([[
        # Channel mix (sum to ~1.0)
        in_store, ue_share, dd_share, sd_share,
        
        # Pricing & cost structure
        commission, delivery_cost, cogs, opex,
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    fi_df = pd.DataFrame({"Feature": FEATURES, "Importance": importances, 
                          "Std": perm_result.importances_std}).sort_values("Importance")
    ...
    st.caption("Permutation importance ± std: average decrease in model performance when each feature is randomly shuffled. Error bars show variability.")
    st.plotly_chart(fig5, use_container_width=True)
    
    # ── Diagnostic: Feature Correlation & Statistics ──
    with st.expander("📊 Feature Importance Diagnostics"):
        diag1, diag2 = st.columns(2)
        with diag1:
            st.markdown("**Importance Distribution**")
            imp_stats = pd.DataFrame({
                "Metric": ["Total Features", "Near-Zero (<0.001)", "Low (0.001-0.01)", "Medium (0.01-0.1)", "High (>0.1)"],
                "Count": [
                    len(importances),
                    (importances < 0.001).sum(),
                    ((importances >= 0.001) & (importances < 0.01)).sum(),
                    ((importances >= 0.01) & (importances < 0.1)).sum(),
                    (importances >= 0.1).sum(),
                ]
            })
            st.dataframe(imp_stats, use_container_width=True, hide_index=True)
        ...
        with diag2:
            ...
            cum_df = pd.DataFrame({
                "Top N Features": range(1, len(sorted_imp) + 1),
                "Cumulative %": cumsum
            })
            fig_cum = px.line(cum_df, x="Top N Features", y="Cumulative %",
                             markers=True, title="Feature Importance: Cumulative Contribution",
                             labels={"Cumulative %": "Cumulative Importance (%)"})
            fig_cum.add_hline(y=80, line_dash="dash", line_color="red", 
                             annotation_text="80% threshold")
            fig_cum.update_layout(plot_bgcolor="white", paper_bgcolor="white",
                                 margin=dict(l=0, r=0, t=40, b=0), height=300)
            st.plotly_chart(fig_cum, use_container_width=True)
        
        st.markdown("---")
        st.markdown("**⚠️ REDUNDANCY ANALYSIS: Which features are stealing importance from each other?**")
        
        # For each near-zero importance feature, find its strongest correlation with high-importance features
        sorted_fi = fi_df.sort_values("Importance", ascending=False)
        top_features = sorted_fi.head(3)["Feature"].tolist()
        zero_features = sorted_fi.tail(10)["Feature"].tolist()  # Show worst 10
        
        redundancy_data = []
        for zero_feat in zero_features:
            max_corr = 0
            most_corr_feat = None
            for top_feat in top_features:
                if zero_feat in st.session_state["X_test"].columns and top_feat in st.session_state["X_test"].columns:
                    c = st.session_state["X_test"][[zero_feat, top_feat]].corr().iloc[0, 1]
                    if abs(c) > max_corr:
                        max_corr = abs(c)
                        most_corr_feat = top_feat
            if most_corr_feat:
                redundancy_data.append({
                    "Near-Zero Feature": zero_feat,
                    "Strongest Correlation": most_corr_feat,
                    "|Correlation|": max_corr
                })
        
        redundancy_df = pd.DataFrame(redundancy_data).sort_values("|Correlation|", ascending=False)
        st.dataframe(redundancy_df, use_container_width=True, hide_index=True)
        
        st.markdown("**Interpretation:**")
        st.info("If near-zero features are **highly correlated (>0.7)** with top features, they're redundant and can be removed. "
               "This is **data leakage/feature engineering issue**, not model accuracy.")
        
        if len(redundancy_df) > 0:
            high_redundant = redundancy_df[redundancy_df["|Correlation|"] > 0.7]
            if len(high_redundant) > 0:
                st.warning(f"🔴 **CRITICAL**: {len(high_redundant)} near-zero features are highly correlated (>0.7) with top features. "
                          "These should be **removed from the feature set** to improve model generalization.")
                st.markdown("**Recommended action:** Consider removing or consolidating these redundant features.")
            else:
                st.success("✓ Near-zero features are NOT correlated with top features. They may genuinely lack predictive power.")
        
        st.markdown("---")
        st.markdown("**Feature Correlation Matrix**")
        num_cols = [col for col in FEATURES if col in st.session_state["X_test"].columns]
        if len(num_cols) > 0:
            corr_subset = st.session_state["X_test"][num_cols].corr()
            fig_corr = px.imshow(corr_subset, title="Feature Correlation (Multicollinearity Check)",
                                color_continuous_scale="RdBu", zmin=-1, zmax=1, text_auto=".2f")
            fig_corr.update_layout(paper_bgcolor="white", margin=dict(l=0, r=0, t=40, b=0), height=400)
            ...
            for i in range(len(corr_subset.columns)):
                for j in range(i+1, len(corr_subset.columns)):
                    if abs(corr_subset.iloc[i, j]) > 0.7:
                        high_corr_pairs.append((corr_subset.columns[i], corr_subset.columns[j], corr_subset.iloc[i, j]))
            if high_corr_pairs:
                high_corr_pairs.sort(key=lambda x: abs(x[2]), reverse=True)
                hc_df = pd.DataFrame({
                    "Feature 1": [x[0] for x in high_corr_pairs],
                    "Feature 2": [x[1] for x in high_corr_pairs],
                    "Correlation": [f"{x[2]:.3f}" for x in high_corr_pairs]
                })
                st.dataframe(hc_df, use_container_width=True, hide_index=True)
                st.error(f"🔴 Found {len(high_corr_pairs)} highly correlated pairs. Your model only needs the top 1-2 from each group.")
            else:
                st.success("✓ No severe multicollinearity detected (|r| ≤ 0.7)")
